...
| `Viewdata` | `0xA3` (see `docs/protocols/viewdata.md`) | Bridges an Econet station to a remote viewdata/videotex server (e.g. Telstar) over a plain TCP connection, one session per station — same LOGIN/DATA/TERMINATE framing as `BeebTerm`, but the far end of the pipe is `React\Socket\Connector` dialing a single configured host:port rather than a named local process |
| `ProxyProvider` | statically configured (see `src/filestored`) | Reserves ports on behalf of a provider hosted in a separate process over the Remote Provider Protocol (see `docs/protocols/remote-provider.md`); not a provider in its own right, just a relay stand-in |
