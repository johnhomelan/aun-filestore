...
/*
 * TypePHP (packaging/typephp): a hand-patched copy of
 * src/vendor/symfony/http-foundation/Response.php - two edits, both no-ops
 * for how the native admin dispatcher uses it:
 *   - dropped the file-scope `class_exists(ResponseHeaderBag::class);`
 *     opcache-preload hint (illegal stray code in tpc bin mode);
 *   - the `public ResponseHeaderBag $headers { set { ... } }` property hook
 *     (PHP 8.4 syntax) reduced to a plain typed property - the hook only
 *     fired a deprecation on direct external assignment, which we never do.
 * The rest of the file is verbatim. Listed in `sources` INSTEAD OF the
 * vendored Response.php.
 */
