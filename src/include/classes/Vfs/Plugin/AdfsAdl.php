 ...
 *
 * @phpstan-type AdfsAdlCatalogueEntry array{load:int,exec:int,size:int,type:string,dir:array<int|string,mixed>}
*/
class AdfsAdl implements PluginInterface {

	/** @var array<string,AdfsReader> */
	 ...
	 */
	public static function setImageReader(string $sImageFile, AdfsReader $oReader): void
	{
	...
	protected static function _getImageReader(string $sImageFile): AdfsReader
	{
	...
	/**
	 * Normalizes the mixed-typed catalogue (or a 'dir' sub-tree of it) from the underlying AdfsReader in to a known shape
	 *
	 * @return array<string,AdfsAdlCatalogueEntry>
	*/
	...
	protected static function _econetToUnix(string $sEconetPath): string
	{
		...
		$sUnixPath = config::getValueAsString('vfs_plugin_localadfsadl_root').DIRECTORY_SEPARATOR.$sUnixPath;
	...
	protected static function _getImageFile(string $sEconetPath): string
	{
		...
		while(count($aUnixPath)>0){
			...
			}elseif($sUnixPath==config::getValueAsString('vfs_plugin_localadfsadl_root')){
				return '';
			}
	...
	protected static function _getPathInsideImage(string $sEconetPath,string $sImageFile): string
	{
		...
		$sPathPreFix = str_ireplace(config::getValueAsString('vfs_plugin_localadfsadl_root'),'',$sPathPreFix);
	...
	public static function _buildFiledescriptorFromEconetPath(User $oUser,FilePath $oEconetPath,bool $bMustExist,bool $bReadOnly): \HomeLan\FileStore\Vfs\FileDescriptor
	{
		...
		if(strlen($sImageFile)>0){
			...
			if($sPathInsideImage===''){
				...
				return new FileDescriptor(self::$oLogger,\HomeLan\FileStore\Vfs\Plugin\AdfsAdl::class,$oUser,$sImageFile,$oEconetPath->getFilePath(),$iVfsHandle,$iEconetHandle,FALSE,TRUE,$bMustExist,$bReadOnly);
			}
			...
			if(AdfsAdl::_checkImageFileExists($sImageFile,$sPathInsideImage)){
				...
				$oAdfs =  AdfsAdl::_getImageReader($sImageFile);
				return new FileDescriptor(self::$oLogger,\HomeLan\FileStore\Vfs\Plugin\AdfsAdl::class,$oUser,$sImageFile,$oEconetPath->getFilePath(),$iVfsHandle,$iEconetHandle,$oAdfs->isFile($sPathInsideImage),$oAdfs->isDir($sPathInsideImage),$bMustExist,$bReadOnly);
			}
		}
	...
	public static function getDirectoryListing(string $sEconetPath,array $aDirectoryListing): array
	{
		...
		if(strlen($sImageFile)>0){
			...
			foreach($aCat as $sFile=>$aMeta){
				$aDirectoryListing[$sFile] = new DirectoryEntry($sFile,$sImageFile,\HomeLan\FileStore\Vfs\Plugin\AdfsAdl::class,$aMeta['load'],$aMeta['exec'],$aMeta['size'],$sEconetPath.'.'.$sFile,$aImageStat['ctime'],'-r/-r', $aMeta['type']=='dir' ? TRUE : FALSE);
			}
		}
			...
			foreach($aFiles as $sFile){
				if(stripos((string) $sFile,'.adl')!==FALSE){
					...
					if(!array_key_exists(substr((string) $sFile,0,strlen((string) $sFile)-4),$aDirectoryListing)){
						...
						$aDirectoryListing[$sFile]=new DirectoryEntry(substr((string) $sFile,0,strlen((string) $sFile)-4),$sFile,\HomeLan\FileStore\Vfs\Plugin\AdfsAdl::class,NULL,NULL,0,$sEconetPath.'.'.substr((string) $sFile,0,strlen((string) $sFile)-4),$aStat['ctime'],'-r/-r', TRUE);
					}
				}
			}
		}
	...
	*/
	...
	{
		if(is_int($fLocalHandle) && array_key_exists($fLocalHandle,AdfsAdl::$aFileHandles)){
	...
	{
		self::$oLogger->debug("AdfsAdl: Write bytes to file handle ".(is_scalar($fLocalHandle) ? (string) $fLocalHandle : gettype($fLocalHandle)));
