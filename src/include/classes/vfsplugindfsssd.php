...
class vfsplugindfsssd {
	...
	protected static function _getPathInsideImage($sEconetPath,$sImageFile)
	{
		...
		$sPathPreFix = str_replace(config::getValue('vfs_plugin_localdfsssd_root'),'',$sPathPreFix);
		$sPathPreFix = str_replace(DIRECTORY_SEPARATOR,'.',ltrim($sPathPreFix,'/'));
		return ltrim(str_replace($sPathPreFix,'',$sEconetPath),'.');
	} 
			...
			if(strlen($sPathInsideImage)>0){
				if(array_key_exists($sPathInsideImage,$aCat)){
					...
					foreach($aCat[$sPathInsideImage] as $sFile=>$aMeta){
						$aDirectoryListing[$sFile] = new directoryentry($sFile,$sImageFile,'vfsplugindfsssd',NULL,NULL,$aMeta['size'],FALSE,$sEconetPath.'.'.$sFile,$aImageStat['ctime'],'-r/-r');
						$aDirectoryListing[$sFile]->setLoadAddr($aMeta['loadaddr']);
						$aDirectoryListing[$sFile]->setExecAddr($aMeta['execaddr']);
					}
				}
			}else{
				$aImageStat = stat($sImageFile);
